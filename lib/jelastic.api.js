...
class JelasticAPI {
  ...
  async get(namespace, section, method, parameters, { raw , rewriteHost} = {}) {
    let options = {
      method: 'GET',
      uri: `${rewriteHost ? rewriteHost : this.host}/JBilling/${namespace}/${section}/${this.type}/${method}?${queryString.stringify(parameters)}`,
      // 'content-type': 'application/x-www-form-urlencoded',
    }
  ...
  async changeGroup(session, appId = this.system_appid, groupName, { uids, sentEmail = false, template }) {
    let result = await this.get('billing', 'account', 'changegroup', {
      session: session,
      appid: appId,
      groupName: groupName,
      uids: uids,
      sentEmail: sentEmail,
      template: template
    }, {rewriteHost: 'https://jca.cloud.ccws.it'}).catch(err => {
      if (this.verbose == true) console.log('> error:', err);
      throw err;
    });
