...
class JelasticAPI {
  ...
  async get(namespace, section, method, parameters) {
    console.log(`${this.host}JBilling/${namespace}/${section}/${this.type}/${method}?${queryString.stringify(parameters)}`)
    let result = await requestPromise({
      method: 'GET',
  ...
  async getAccountConsumptionByPeriodInner(session, appId, startTime, endTime, uid) {
    console.log({
      uid: uid,
      session: session,
      appid: appId,
